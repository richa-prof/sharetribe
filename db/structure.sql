...
CREATE TABLE `communities` (
  ...
  `users_can_invite_new_users` tinyint(1) DEFAULT '0',
...
CREATE TABLE `custom_field_option_selections` (
  ...
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `index_custom_field_option_selections_on_custom_field_value_id` (`custom_field_value_id`),
...
CREATE TABLE `listings` (
  ...
  `created_at` datetime DEFAULT NULL,
  `updates_email_at` timestamp NULL DEFAULT NULL,
  `updated_at` datetime DEFAULT NULL,
  ...
  KEY `index_listings_on_listing_type_old` (`listing_type_old`),
  KEY `index_listings_on_open` (`open`),
  KEY `index_listings_on_category_old` (`category_old`),
  KEY `index_listings_on_share_type_old` (`share_type_old`),
...
CREATE TABLE `transaction_transitions` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `index_transaction_transitions_on_sort_key_and_transaction_id` (`sort_key`,`transaction_id`),
  KEY `index_transaction_transitions_on_transaction_id` (`transaction_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
